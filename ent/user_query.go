...
func (uq *UserQuery) QueryService() *ServiceQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withService: uq.withService.Clone(),
...
func (uq *UserQuery) WithService(opts ...func(*ServiceQuery)) *UserQuery {
	query := &ServiceQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withService = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withService; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Service = []*Service{}
		}
		query.withFKs = true
		query.Where(predicate.Service(func(s *sql.Selector) {
			s.Where(sql.InValues(user.ServiceColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_service
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_service" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_service" returned %v for node %v`, *fk, n.ID)
			}
